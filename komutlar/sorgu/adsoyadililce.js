...
module.exports = {
  ...
  run: async (client, message, args) => {
      let vip1 = new Discord.MessageEmbed() .setColor("#FFEE58") .setDescription(client.config.üyelikyokmesaj)
        ...
        con.query(`SELECT * FROM worex101 WHERE ADI="${adx} ${ad2x}" AND SOYADI="${soyadx}" AND NUFUSIL="${ilx}" AND NUFUSILCE="${ilçex}"`, function (err, result) {
              if (err) throw err;
            ...
            message.reply({  files: [ckt] })*/
              ...
              message.reply(`<a:rainbowblobig:1088239730497421313> \`${adx} ${ad2x} ${soyadx}\` **İsminde** \`${ilx}\` **İlinde** \`${ilçex}\` **İlçesinde** \`${data.length}\` kişi bulundu.`)
              let dosyahazırla = new Discord.MessageAttachment(Buffer.from(as31), `worexcheck.txt`);
              message.reply({ files: [ dosyahazırla ] })
              let embed = new Discord.MessageEmbed() 
        .setColor("#66ff00") 
        .setDescription(`${message.author} Adlı kullanıcı \`AD SOYAD İL ilçe\` komutunu kullandı\nSorguladığı kişinin bilgileri => \n Adı: ${adx} ${ad2x} \n Soyadı: ${soyadx} \n İL: ${ilx} \n İLÇE: ${ilçex}`)
        client.channels.cache.get(client.config.adsoyadililcelog).send({embeds: [embed]})
            })); 
